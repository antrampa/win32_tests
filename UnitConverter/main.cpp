...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        // Weight
        if (LOWORD(wParam) == 1) {
            GetWindowText(hWeightInput, buffer, 100);
            value = _wtof(buffer);
            ...
            SetWindowText(hWeightResult, (L"Result: " + std::to_wstring(result)).c_str());
        }

        // Length
        if (LOWORD(wParam) == 2) {
            GetWindowText(hLengthInput, buffer, 100);
            value = _wtof(buffer);
            ...
            SetWindowText(hLengthResult, (L"Result: " + std::to_wstring(result)).c_str());
        }

        // Volume
        if (LOWORD(wParam) == 3) {
            GetWindowText(hVolumeInput, buffer, 100);
            value = _wtof(buffer);
            ...
            SetWindowText(hVolumeResult, (L"Result: " + std::to_wstring(result)).c_str());
        }
